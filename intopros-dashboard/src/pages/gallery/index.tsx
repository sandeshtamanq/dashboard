...
function GalleryList() {
  ...
  const deleteBlog = async (id: number) => {
    const res = await makeRequest(`/gallery/${id}`, {
      method: "DELETE",
    });
    if (res) {
      notification.success({ message: "Successfully deleted gallery item!" });
  ...
  const fetchItems = async (pageNum?: number) => {
    ...
    setIsLoading(true);
    const res = await makeRequest(`/gallery?${params.toString()}`);
    if (res) {
    ...
    <div className="mt-10">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Row gutter={10}>
            {galleryItems.map((gallery: any, index) => (
              <Col key={index} span={24} style={{ marginBottom: 10 }}>
                ...
                >
                  <Space>
                    {gallery?._images?.map((image: string, index: number) => (
                      <Image
                        key={index}
                  ...
                  <div>
                    <Space>
                      <Button
                        ...
                        size="small"
                        onClick={() =>
                          router.push(`/gallery/${gallery.id}/edit`)
                        }
                      >
                        Edit
                      </Button>
                      ...
                      <Popconfirm
                        ...
                        onConfirm={() => deleteBlog(gallery.id)}
                        okText="Yes"
                        cancelText="No"
                      >

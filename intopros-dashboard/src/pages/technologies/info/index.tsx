...
function TechnologyInfoList() {
  ...
  const deleteInfo = async (id: number) => {
    const res = await makeRequest(`/technologies/info/${id}`, {
      method: "DELETE",
    });
    if (res) {
      notification.success({ message: "Successfully deleted Info!" });
  ...
  const fetchItems = async (pageNum?: number, filterBy?: string) => {
    ...
    setIsLoading(true);
    const res = await makeRequest(`/technologies/info?${params.toString()}`);
    if (res) {
  ...
  return (
    <div className="mt-10">
      <Typography.Title level={3}>Technologies Info</Typography.Title>
        ...
        <Row justify="space-between">
          ...
          <Col>
            <Space>
              ...
              <Button
                type="primary"
                onClick={() => router.push("/technologies/info/add")}
                icon={<PlusOutlined />}
              >
                <span>Add</span>
              </Button>
      ...
      {isLoading ? (
        ...
        <>
          <div style={{ textAlign: "center" }}>
            {technologiesInfo.length < 1 && (
              <Typography.Title level={5}>No Info found!</Typography.Title>
            )}
          </div>

          <Row gutter={10}>
            {technologiesInfo.map((info: any, index) => (
              <Col key={index} span={24} style={{ marginBottom: 10 }}>
                ...
                >
                  ...
                  <Typography.Paragraph>
                    <span>
                      {removeHtmlTags(info.description).substring(0, 500)}
                    </span>
                    <span>
                      <strong>...</strong>
                    </span>
                  ...
                  <div>
                    <Space>
                      <Button
                        ...
                        size="small"
                        onClick={() =>
                          router.push(`/technologies/info/${info.id}/edit`)
                        }
                      >
                        Edit
                      </Button>
...
export default TechnologyInfoList;

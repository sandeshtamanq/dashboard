import {
  ...
  export default function CareerCategoryForm(props: IProps) {
    ...
    const saveCareerCategoryDetails = (values: any) => {
 
      // return console.log(values, uploadedImg, isImageDeleted);
  
      makeRequest(props.mode === "add" ? "/careers/category" : `/careers/category/${router.query.id}`, {
        ...
        if (res) {
          const notificationMsg =
            props.mode === "add"
              ? "Successfully created Career Category"
              : "Succefully updated Career Category";
  
          notification.success({ message: notificationMsg });
          router.push("/careers/categories");
        }
      });
    };
    ...
    return (
      <Form
        labelCol={{ span: 24 }}
        form={form}
        onFinish={saveCareerCategoryDetails}
        onFinishFailed={() => {
          notification.error({
            message:
      ...
      >
        <Row justify="space-between" align="middle" className="action-bar">
          <Typography.Title level={3}>
            {props.mode === "add" ? "Add Career" : "Edit Career"}
          </Typography.Title>

...
export default function CareerForm(props: IProps) {
  ...
  const saveCareerDetails = (values: any) => {
    ...
    makeRequest(
      props.mode === "add" ? "/careers" : `/careers/${router.query.id}`,
      {
  ...
  const fetchItems = async () => {
    const res = await makeRequest(`/careers/category?take=0`);
    if (res) {
